...
#py.sign_in('anirudhhravi1998', 'fMw6hcVfzdMTfBMqrcuA')
...
# Files
DATA_SET_PATH = "E:\vg\input.csv"


def dataset_headers(dataset):                                         #gets the input headding names
    """
...
def unique_observations(dataset, header, method=1):                      # by default the algorithm chooses to run logistic regression but depending on the input it may choose numpy 
    """
...
def feature_target_frequency_relation(dataset, f_t_headers):                                  
    ...
    for feature in feature_unique_observations:
        frequencies[feature] = {unique_targets[0]: len(dataset[(dataset[f_t_headers[0]] == feature) & (dataset[f_t_headers[1]] == unique_targets[0])]),unique_targets[1]: len(dataset[(dataset[f_t_headers[0]] == feature) & (dataset[f_t_headers[1]] == unique_targets[1])])}
    return frequencies


def feature_target_histogram(feature_target_frequencies, feature_header):                    #finding the frequency relation for various calculation parameters
    """
...
def train_logistic_regression(train_x, train_y):                                                                #training using logistic regression
    """
...
def model_accuracy(trained_model, features, targets):                                 #predicting the accuracy of the trained data set using metrics
    """
...
def main():                                                                                  #main code
    ...
    dataset = pd.read_csv("E:\vg\input.csv")
